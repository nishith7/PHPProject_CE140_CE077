...
// 		}
		...
		function editMobile(Mobile $mobile){
			...
			$sql = "UPDATE mobile SET model='$model',brandID='$brandID',price='$price',mobileUrl='$imageUrl',cameraSpecs='$cameraSpecs',memorySpecs='$memorySpecs',
		...
		function addMobile(Mobile $mobile){
			...
			$sql = "INSERT INTO mobile (model,brandID,price,mobileUrl,cameraSpecs,memorySpecs,networkSpecs,platform,cpu,features)
				VALUES ('$model','$brandID','$price','$imageUrl','$cameraSpecs','$memorySpecs','$networkSpecs','$platform','$cpu','$features')";
		...
		function getMobileById($id){
			...
			if($result->num_rows > 0){
				while($row = $result->fetch_assoc()){
					...
					$mobile = new Mobile($ID, $model, $brandID, $price, $imageUrl,$cameraSpecs, $memorySpecs, $networkSpecs, $platform, $cpu, $features);
				}
				return $mobile;
			}
			return 0;
